...
if (isset($_GET['id'])) {
    try {
        ...
        // SQL文の実行結果を配列で取得する
        // 補足：1つのレコード（横1行のデータ）のみを取得したい場合、メソッドを使えばカラム名がキーになった1次元配列を取得できる
        $product = $stmt_select_product->fetch(PDO::FETCH_ASSOC);

        // idパラメータの値と同じのデータが存在しない場合はエラーメッセージを表示して処理を終了する
        //補足：fetch（）メソッドは実行結果が取得出来なかった場合にFALSEを返す
        if ($product === FALSE) {
            exit('idパラメータの値が不正です。');
        }
                ...
                <div>
                    ...
                    <select id="vendor_code" name="vendor_code" required>
                        ...
                        foreach ($vendor_codes as $vendor_code) {
                            // もし変数が商品の仕入れ先コードの値と一致していれば、属性をつけて初期値にする
